...
{
    private readonly ITokenEncryptionService _tokenEncryption = new TokenEncryptionService();
    ...
    protected override async void OnLoaded(RoutedEventArgs e)
    {
        ...
        if (File.Exists("apiToken"))
        {
            ...
            {
                var encryptedToken = System.Text.Json.JsonSerializer.Deserialize<string>(text);
                if (!string.IsNullOrEmpty(encryptedToken))
                {
                    var decryptedToken = _tokenEncryption.Decrypt(encryptedToken);
                    ApiTextBox.Text = decryptedToken;
                }
            }
        }
    ...
    private void MarkHeroes(List<With> heroes)
    {
        foreach (var logical in this.GetLogicalDescendants())
        {
            ...
            try
            {
                ...
                LoggingService.Logger.Debug("Marked hero {HeroId} as suggestion", herobtn.HeroId);
            }
            catch (Exception ex)
            {
                LoggingService.Logger.Error(ex, "Error marking hero {HeroId}", herobtn.HeroId);
            }
        }
    }
    ...
    private async void ConfirmButton_OnClick(object? sender, RoutedEventArgs e)
    {
        ...
        {
            LoggingService.Logger.Warning("API token is empty");
            return;
        }
        ...
        if (isValidToken)
        {
            LoggingService.Logger.Information("API token is valid");
            App.ConnectApi(validApiToken);
            if (RememberMeCheckBox.IsChecked == true)
            {
                var encryptedToken = _tokenEncryption.Encrypt(validApiToken);
                var serializeObject = System.Text.Json.JsonSerializer.Serialize(encryptedToken);
                await File.WriteAllTextAsync("apiToken", serializeObject, Encoding.UTF8);
                LoggingService.Logger.Information("API token saved to file");
            }
        ...
        {
            LoggingService.Logger.Warning("API token is invalid");
        }
    }
    ...
    private Task<bool> ApiValidation(string apiToken)
    {
        ...
        try
        {
            ...
            LoggingService.Logger.Debug("API token payload: {Payload}", json);
            ...
            if (doc.RootElement.TryGetProperty("exp", out var exp))
            {
                ...
                {
                    LoggingService.Logger.Warning("API token expired: {ExpirationTime}", expTime);
                    return Task.FromResult(false);
                }
                LoggingService.Logger.Information("API token valid until: {ExpirationTime}", expTime);
            }
        ...
        {
            LoggingService.Logger.Error(ex, "API token validation failed");
            return Task.FromResult(false);
        }
    }
