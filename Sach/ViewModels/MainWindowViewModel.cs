...
public class MainWindowViewModel : ViewModelBase
{
    ...
    public async Task GetHeroStats()
    {
        ...
        try
        {
            ...
            for (int attempt = 0; attempt < 5; attempt++)
            {
                ...
                {
                    if (attempt == 4) throw;
                    Console.WriteLine($"Сервер не готов, попытка {attempt + 1}/5...");
            ...
            var advantage = json.RootElement
                .GetProperty("data")
                .GetProperty("heroStats")
                .GetProperty("heroVsHeroMatchup")
                .GetProperty("advantage")[0];

            List<With> stats;
            if (SelectedHero.IsAlly)
            {
                stats = advantage.GetProperty("with").EnumerateArray()
                    .Select(x => new With
                    {
            ...
            else if (SelectedHero.IsEnemy)
            {
                stats = advantage.GetProperty("vs").EnumerateArray()
                    .Select(x => new With
                    {
                        HeroId2 = x.GetProperty("heroId2").GetInt16(),
                        Synergy = -x.GetProperty("synergy").GetDecimal()
    ...
    private async Task<List<With>> GetHeroesStats(List<Hero> heroes)
    {
        ...
        foreach (var hero in heroes)
        {
            ...
            {
                Console.WriteLine($"Ошибка для героя {hero.HeroId}: {ex.Message}");
            }
        }
    ...
    {
        Proxy = new WebProxy("127.0.0.1", 12334)
        {
